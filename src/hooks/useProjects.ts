...
function toPathList(arr: ImageItem[]): string[] {
  return arr
    .map((img) => {
...
function getImagePaths(raw: Record<string, unknown>): string[] {
  // Prefer preview_images (admin-uploaded card previews) over regular images
  const preview = raw.preview_images as ImageItem[] | undefined;
  if (Array.isArray(preview) && preview.length > 0) {
    return toPathList(preview);
  }
  const images = raw.images as ImageItem[] | undefined;
  if (Array.isArray(images) && images.length > 0) {
    return toPathList(images);
  }
