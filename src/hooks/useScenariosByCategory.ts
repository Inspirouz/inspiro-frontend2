...
function getFirstImageUrl(raw: RawItem): string {
  const img = (raw.image ?? raw.logo ?? raw.cover ?? raw.thumbnail ?? raw.path) as
    | string
    | undefined;
  if (img) return toImageUrl(img);
  const images = raw.images as
    | string[]
    | Array<{ path?: string; file_name?: string; url?: string }>
    | undefined;
  if (Array.isArray(images) && images.length > 0) {
    ...
    if (p) return toImageUrl(String(p));
  }
  return '';
}

function mapToContentItem(raw: RawItem, index: number): ContentItem {
  const inner = (raw.project ?? raw.screen ?? raw) as Record<string, unknown>;
  const id = String(inner.id ?? raw.id ?? raw.slug ?? `scenario-${index}`);
  const name = String(
    inner.app_name ?? inner.name ?? inner.title ?? raw.app_name ?? raw.name ?? raw.title ?? ''
  );
  const logoStr = (inner.logo ?? inner.image ?? raw.logo ?? raw.image ?? raw.cover) as
    | string
    | undefined;
...
/**
 * Fetches scenarios for a category from GET /scenarios?category_id=:categoryId.
 * When categoryId is null or 'all', returns an empty array (caller falls back to contentData).
 */
export function useScenariosByCategory(categoryId: string | null) {
