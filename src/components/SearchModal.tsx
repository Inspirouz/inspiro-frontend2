...
const SEARCH_TYPES: SearchType[] = ['applications', 'ui_elements', 'scenarios', 'patterns', 'fonts'];
...
const SearchModal = ({ onClose }: { onClose?: () => void }) => {
  ...
  const fetchSearch = useCallback(async (q: string, type: SearchType) => {
    const apiUrl = import.meta.env.VITE_API_URL;
    setLoading(true);
    setError(null);
    try {
      ...
      const list = Array.isArray(data?.data) ? data.data : data?.data?.[type] ?? [];
      setItems(
        list.map((x: { id?: string; name?: string; category?: string; icon?: string; type?: string }) => ({
          id: String(x.id ?? ''),
          name: x.name ?? '',
          category: x.category,
          icon: x.icon ?? '',
          type: (x.type as SearchType) ?? type,
        }))
      );
    } catch (err) {
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      fetchSearch(searchQuery, activeFilter);
    }, 300);
    return () => clearTimeout(t);
  }, [searchQuery, activeFilter, fetchSearch]);

  const filterIcons: Record<SearchType, React.ReactNode> = {
    applications: <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    ui_elements: <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    scenarios: <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    patterns: <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    fonts: <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            {items?.map((item) => (
              <li key={item.id} className="search-modal__item" style={{ cursor: 'pointer' }} onClick={() => { navigate(`/detail/${item.id}`); onClose?.(); }}>
                <div className="search-modal__item-icon">
                  {item.icon ? (
                    <img src={toImageUrl(item.icon)} alt={item.name} width={48} height={48} />
                  ) : (
